...
| Use official ERC-8004 singletons                      | Identity Registry `0x8004A169FB4a3325136EB29fA0ceB6D2e539a432` and Reputation Registry `0x8004BAa17C55a88189AE136b182e5fdA19dE9b63` are deployed at the **same address on all chains** (CREATE2). The Identity Registry address is passed to the `AgentRegistry` constructor (immutable); `erc8004AgentId` is used for reputation. | May 2026 |
| Deploy our own ValidationRegistry                     | Validation Registry is still under active revision upstream — we deploy our own alongside `AgentRegistry`                                                                                                                                                                                                                          | May 2026 |
...
| `REPUTATION_REGISTRY_ADDRESS`      | No       | Overrides the official ERC-8004 Reputation Registry address (default: `0x8004BAa17C55a88189AE136b182e5fdA19dE9b63`) |
...
- [ ] Deploy `ValidationRegistry` to Base Sepolia (set `VALIDATION_REGISTRY_ADDRESS` env var via `npm run setup-env:baseSepolia`)
