...
Set `NETWORK=base` or `NETWORK=baseSepolia` in `apps/oracle/.env`. The dashboard uses both public networks simultaneously — switch via the RainbowKit network selector.
...
| `TeeVerifier`        | ERC-7857 | `IAgentDataVerifier` — verifies TDX DCAP quotes from the oracle        |
...
DCAP mode is fixed by network:

- Local always uses fake DCAP.
- Base Sepolia can use fake or real DCAP.
...
| `deployments.json`                     | No       | Public deployed contract addresses, including `verifier` and `teeVerifier`, plus AgentRegistry scan start blocks |
